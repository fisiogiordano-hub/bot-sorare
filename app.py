...
def check_account():
    ...
    print(
        f"✅ Sorare: {u.get('nickname') or u.get('slug')}",
        flush=True
    )
    return True
...
def card_price(card):
    ...
    for key in (
        ...
        try:
            x=(
                (s.get("liveSingleSaleOffer") or {})
                .get("receiverSide") or {}
            ).get("amounts",{}).get("eurCents")
...
def get_competitions(card):
    ...
    return list(dict.fromkeys(
        ...
        if isinstance(c,dict) and slug(c.get("slug"))
    ))
...
def counter_offer(offer,cards):
    ...
    print(
        f"🔐 Autorizzazioni ricevute: {len(auths)}",
        flush=True
    )
...
def process_offer(offer):
    ...
    # Carta non valida = comportamento originale:
    # rifiuta l'offerta.
    if not valid:
        print(
    ...
    if ok:
        ...
        # Solo se la controproposta è riuscita,
        ...
        # IMPORTANTE:
        # se la controproposta fallisce,
        # NON rifiutiamo l'offerta.
        print(
