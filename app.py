...
def inspect_prepare_schema():
    ...
    data = graphql(query)
    typ = ((data or {}).get("data") or {}).get("__type")

    if not typ:
        print("⚠️ Impossibile leggere prepareOfferInput", flush=True)
        return

    print("🔍 SCHEMA LIVE prepareOfferInput:", flush=True)

    for field in typ.get("inputFields") or []:
        t = field.get("type") or {}
        parts = []
        ...
        print(f"   • {field.get('name')}: {' → '.join(parts)}", flush=True)
...
def card_details(asset_ids):
    asset_ids = list(dict.fromkeys(str(x).strip() for x in asset_ids if x))
    if not asset_ids:
        return []
...
def card_price(card):
    ...
    for name in ("lowestPriceCard", "lowestPriceCardAnySeason"):
...
def valid_card(card):
    ...
    if age >= MAX_AGE:
        print(f"      ❌ Età troppo alta (limite: < {MAX_AGE})", flush=True)
        return False
...
def reject_offer(offer):
    ...
    if not result:
        print("❌ Risposta rejectOffer vuota", flush=True)
        return False
...
def counter_offer(offer, cards):
    ...
    if not receiver:
        print("❌ receiverSlug mancante", flush=True)
        return False

    if not asset_ids:
        print("❌ Nessuna carta da ricevere", flush=True)
        return False
    ...
    if not STARK:
        print("❌ SORARE_STARK_PRIVATE_KEY mancante", flush=True)
        return False

    # IMPORTANTE:
    # exchangeRateId NON viene inserito qui.
    prepare_input = {
        "sendAssetIds": [],
    ...
    if not data:
        print("❌ Nessuna risposta da prepareOffer", flush=True)
        return False
    ...
    if not result:
        print("❌ prepareOffer ha restituito NULL", flush=True)
        return False
    ...
    if not authorizations:
        print("❌ Nessuna autorizzazione restituita", flush=True)
        return False
    ...
    if not data:
        print("❌ Nessuna risposta da createDirectOffer", flush=True)
        return False
    ...
    if not result:
        print("❌ createDirectOffer ha restituito NULL", flush=True)
        return False
    ...
    if not offer_id:
        print("❌ Nessuna offerta creata da Sorare", flush=True)
        return False
...
def process_offer(offer):
    ...
    if not any(is_kulenovic(c) for c in receiver_cards):
        print("⏭️ Kulenovic non presente: ignoro", flush=True)
        return
    ...
    if not ids:
        print("❌ Nessuna carta offerta", flush=True)
        return
    ...
    if rejected:
        print(f"⚠️ {rejected} carta/e esclusa/e", flush=True)
    ...
    if result:
        ...
        print(
            "🟡 Offerta originale lasciata IN SOSPESO per nuovo tentativo.",
            flush=True,
        )
...
def worker():
    ...
    if not check_account():
        print("❌ Account non valido. Worker fermato.", flush=True)
        return

    # Legge lo schema reale dell'API live.
    inspect_prepare_schema()
