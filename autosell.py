...
VERSION = "AUTOSell-12.0-SOLANA-FIX"
...
def get_floor(card):
    ...
    for offer in nodes:
        ...
        for listed in sender.get("anyCards") or []:
            ...
            try:
                if int(listed.get("seasonYear")) != season:
                    continue
            except Exception:
                continue
...
def sign_authorizations(authorizations):
    ...
    for auth in authorizations:
        typ = (auth.get("request") or {}).get("__typename")

        if (
            typ != "SolanaTokenTransferAuthorizationRequest"
            and not STARK
        ):
            raise RuntimeError(
        ...
        if typ == "SolanaTokenTransferAuthorizationRequest" and not SOLANA:
            raise RuntimeError(
...
async function sol(auth) {
    ...
    /*
     * createKeyPairSignerFromBytes supporta
     * direttamente la keypair Solana da 64 byte
     * oppure la private key da 32 byte.
     */
    const signer =
        await createKeyPairSignerFromBytes(key);
...
function stark(auth) {
    ...
    if (
        ...
        base.starkexLimitOrderApproval = {
            nonce: req.nonce,
            expirationTimestamp:
                req.expirationTimestamp,
            signature
    ...
    if (
        ...
        base.mangopayWalletTransferApproval = {
            nonce: req.nonce,
            signature
        };
        return base;
