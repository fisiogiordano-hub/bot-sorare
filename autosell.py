...
def sellable_cards():
    ...
    for card in get_cards():
        if not isinstance(card,dict):
            continue
        if norm(card.get("status")) not in {"da_vendere","ready"}:
            continue
        if asset_id(card):
...
def gql(query,variables=None):
    for attempt in range(3):
        try:
            response = requests.post(SORARE_URL,headers=auth_headers(),json={"query":query,"variables":variables or {}},timeout=TIMEOUT)
...
def check_account():
    data = gql("""query { currentUser { slug nickname starkKey } }""")
    user = ((data or {}).get("data") or {}).get("currentUser")
    if not user:
        print("❌ Account Sorare non verificato",flush=True)
        return False
    print("✅ Sorare: " + str(user.get("nickname") or user.get("slug")),flush=True)
...
def card_details(asset):
    ...
    """,{"ids":[asset]})
    cards = ((data or {}).get("data") or {}).get("anyCards") or []
...
def find_active_public_offer(asset):
        ...
        if not errors:
            ...
            for card in cards:
                ...
                if offer_id:
                    ...
                    print(f"🟢 OFFER → {offer_id}",flush=True)
                    return offer_id
        else:
            print("⚠️ Pre-check mirato non disponibile:",json.dumps(errors,ensure_ascii=False)[:1500],flush=True)
    ...
    errors = data.get("errors") or []
    if errors:
        print("⚠️ Pre-check fallback:",json.dumps(errors,ensure_ascii=False)[:1500],flush=True)
        return None
    ...
    for offer in nodes:
        ...
        for listed_card in cards:
            if not isinstance(listed_card,dict):
                continue
            if norm(listed_card.get("assetId")) == wanted:
                offer_id = offer.get("id")
                print("🟢 PRE-CHECK → CARTA GIÀ IN VENDITA",flush=True)
                print(f"🟢 OFFER → {offer_id}",flush=True)
                return offer_id
...
def usd_to_eur(usd_cents):
    try:
        response = requests.get("https://api.frankfurter.app/latest",params={"from":"USD","to":"EUR"},timeout=10)
...
def get_floor(card):
    ...
    """,{"slug":player_slug,"first":50})
    nodes = ((((data or {}).get("data") or {}).get("tokens") or {}).get("liveSingleSaleOffers") or {}).get("nodes") or []
    prices = []
    for offer in nodes:
        ...
        for listed in sender.get("anyCards") or []:
            ...
            if norm(listed.get("rarityTyped")) != rarity:
                continue
            price = amount_to_eur((offer.get("receiverSide") or {}).get("amounts"))
...
def get_eth_eur_rate():
    try:
        response = requests.get("https://api.coingecko.com/api/v3/simple/price",params={"ids":"ethereum","vs_currencies":"eur"},timeout=10)
        response.raise_for_status()
        data = response.json()
        rate = float(data["ethereum"]["eur"])
        if rate <= 0:
            raise ValueError("Cambio ETH/EUR non valido")
        print(f"💱 ETH/EUR → €{rate:.2f}",flush=True)
        return rate
    except Exception as e:
        print(f"❌ Impossibile ottenere ETH/EUR: {e}",flush=True)
        return None

def eth_to_eur_cents(eth_amount,eth_eur_rate):
    try:
        value = float(eth_amount) * float(eth_eur_rate)
        return max(1,round(value * 100))
    except Exception:
        return None

def is_kulenovic(card):
    return norm(card.get("slug")) == norm(KULENOVIC_SLUG) or norm(card.get("assetId")) == norm(KULENOVIC_ASSET)
...
def base58_decode(value):
    ...
    raw = b"" if number == 0 else number.to_bytes(max(1,(number.bit_length() + 7) // 8),"big")
    zeros = 0
    for char in value:
        if char != "1":
            break
...
def solana_key_info():
    ...
    raise RuntimeError("SORARE_SOLANA_PRIVATE_KEY non valida. Atteso Base58 o HEX da 32/64 byte.")

def sign_authorizations(authorizations):
    node = shutil.which("node") or shutil.which("nodejs")
    if not node:
        raise RuntimeError("Node.js non disponibile")
    types = [(a.get("request") or {}).get("__typename") for a in authorizations]
    requires_stark = any(t not in {"SolanaTokenTransferAuthorizationRequest"} for t in types)
    requires_solana = any(t == "SolanaTokenTransferAuthorizationRequest" for t in types)
    if requires_stark and not STARK:
        raise RuntimeError("SORARE_STARK_PRIVATE_KEY mancante per authorization Stark/Mangopay")
...
function b58decode(value) {
    ...
    let bytes;
    if (num === 0n) bytes = Buffer.alloc(0);
    else {
...
function parsePrivateKey(value) {
    ...
    if (/^[0-9a-fA-F]+$/.test(hex) && hex.length % 2 === 0) {
        const decoded = new Uint8Array(Buffer.from(hex, "hex"));
        if (decoded.length === 32 || decoded.length === 64) return decoded;
    }
...
async function createSolanaSigner(privateKey) {
    ...
    if (keyBytes.length !== 32) throw new Error("Private key Solana non valida: " + keyBytes.length + " byte");
...
async function signSolana(auth) {
    ...
    const signer = await createSolanaSigner(input.solanaPrivateKey);
    console.error("🔑 Solana signer → " + signer.address);
    console.error("🎯 senderAddress → " + req.senderAddress);
    if (signer.address !== req.senderAddress) throw new Error("La chiave Solana NON corrisponde al senderAddress. Derivato=" + signer.address + " richiesto=" + req.senderAddress);
    const message = ["TRANSFER", req.transferProxyProgramAddress, req.merkleTreeAddress, req.leafIndex.toString(), req.nonce, req.expirationTimestamp.toString(), req.receiverAddress, "0x", req.originator].join(":");
    ...
    const signatureBytes = signatures[0][signer.address];
    return { fingerprint: auth.fingerprint, solanaTokenTransferApproval: { signature: b58encode(signatureBytes), nonce: req.nonce, expirationTimestamp: req.expirationTimestamp } };
}
function signStark(auth) {
    ...
    if (req.__typename === "StarkexTransferAuthorizationRequest") return { ...base, starkexTransferApproval: { nonce: req.nonce, expirationTimestamp: req.expirationTimestamp, signature } };
    if (req.__typename === "StarkexLimitOrderAuthorizationRequest") return { ...base, starkexLimitOrderApproval: { nonce: req.nonce, expirationTimestamp: req.expirationTimestamp, signature } };
    if (req.__typename === "MangopayWalletTransferAuthorizationRequest") return { ...base, mangopayWalletTransferApproval: { nonce: req.nonce, signature } };
...
async function main() {
    ...
    for (const auth of input.authorizations) {
        ...
        approvals.push(type === "SolanaTokenTransferAuthorizationRequest" ? await signSolana(auth) : signStark(auth));
    }
...
'''
    process = subprocess.run([node,"-e",js],input=json.dumps({"starkPrivateKey":STARK,"solanaPrivateKey":SOLANA,"authorizations":authorizations}),text=True,capture_output=True,timeout=TIMEOUT)
    if process.stderr:
    ...
    try:
        ...
        raise RuntimeError("Output firma non valido: " + process.stdout[:1000])
...
def prepare_sale(asset,price):
    ...
    data = gql(PREPARE_QUERY,{"input":input_data})
    ...
    if errors:
        print("❌ prepareOffer:",json.dumps(errors,ensure_ascii=False),flush=True)
        return None
    authorizations = result.get("authorizations") or []
    if not authorizations:
        print("❌ prepareOffer: nessuna authorization",flush=True)
        return None
    print(f"✅ Authorization ricevute: {len(authorizations)}",flush=True)
...
def create_sale_once(card,price):
    ...
    print(f"📤 createSingleSaleOffer → EUR {eur(price)}",flush=True)
    data = gql(create_query,{"input":create_input})
    result = ((data or {}).get("data") or {}).get("createSingleSaleOffer")
    if not result:
        print("❌ createSingleSaleOffer: nessun risultato",flush=True)
        return None,None
    errors = result.get("errors") or []
    if errors:
        error_text = " ".join(str(e.get("message","")) for e in errors if isinstance(e,dict))
        if "active public offer already exists" in norm(error_text):
            ...
            if existing_offer:
                ...
                return existing_offer,"ALREADY-LISTED"
            return "ALREADY-LISTED","ALREADY-LISTED"
        print("❌ createSingleSaleOffer:",json.dumps(errors,ensure_ascii=False),flush=True)
...
def is_technical_price_error(error_text):
    ...
    return "price must be greater than" in text or "price must be at least" in text or "minimum price" in text or "min price" in text

def create_sale(card,price):
    asset = asset_id(card)
    if not asset:
        return None
    print(f"🎯 TENTATIVO FLOOR → {eur(price)}",flush=True)
    offer_id,error = create_sale_once(card,price)
    if offer_id:
        return offer_id
    if not is_technical_price_error(error or ""):
        print("❌ Vendita fallita per errore non legato al minimo tecnico.",flush=True)
        return None
    print("⚠️ FLOOR SOTTO IL MINIMO TECNICO SORARE",flush=True)
    eth_eur_rate = get_eth_eur_rate()
    if eth_eur_rate is None:
        print("❌ Impossibile determinare il prezzo EUR del minimo tecnico.",flush=True)
        return None
    technical_eth = TECHNICAL_START_ETH
    max_attempts = 1000
    for attempt in range(max_attempts):
        technical_price = eth_to_eur_cents(technical_eth,eth_eur_rate)
        if technical_price is None:
            print("❌ Conversione ETH/EUR fallita",flush=True)
            return None
        print(f"🔁 TENTATIVO MINIMO TECNICO → {technical_eth:.4f} ETH ≈ {eur(technical_price)}",flush=True)
        offer_id,error = create_sale_once(card,technical_price)
        if offer_id:
            print(f"✅ MINIMO TECNICO TROVATO → {technical_eth:.4f} ETH ≈ {eur(technical_price)}",flush=True)
            return offer_id
        if not is_technical_price_error(error or ""):
            print("❌ Tentativo tecnico fallito per errore non legato al prezzo minimo.",flush=True)
            return None
        technical_eth += TECHNICAL_STEP_ETH
        technical_eth = round(technical_eth,4)
    print("❌ Raggiunto limite massimo dei tentativi tecnici.",flush=True)
    return None
...
def process(card):
    ...
    if existing_offer:
        ...
        print(f"🟢 OFFER → {existing_offer}",flush=True)
        update_card(asset,status="SELLING",offer_id=existing_offer,error=None)
        print("🟢 STATO → SELLING (offerta già presente)",flush=True)
        return
    ...
    if not ok:
        messages = {"KULENOVIC":"KULENOVIC PROTETTO","RARITY":"RARITÀ NON LIMITED","FLOOR_UNKNOWN":"FLOOR NON DISPONIBILE","FLOOR_HIGH":"FLOOR SOPRA €0.70"}
        print(f"🚫 ESCLUSA → {messages.get(result,result)}",flush=True)
        update_card(asset,status=("BLOCKED" if result in {"KULENOVIC","RARITY"} else "da_vendere"),error=result)
        return
    price = result
    print(f"✅ CARTA VALIDA → floor {eur(price)}",flush=True)
    ...
    if not offer_id:
        ...
        print("🔁 Carta rimessa in DA_VENDERE",flush=True)
        return
    update_card(asset,status="SELLING",offer_id=offer_id,error=None)
    if offer_id == "ALREADY-LISTED":
        print("🟢 CARTA GIÀ IN VENDITA",flush=True)
    elif offer_id == "DRY-RUN":
        print("🟡 DRY RUN COMPLETATO",flush=True)
    else:
        print(f"🎉 AUTOSELL COMPLETATO → {label(details)} | offer={offer_id}",flush=True)
    print("🟢 STATO → SELLING",flush=True)

def recovery():
    selling = [card for card in get_cards() if norm(card.get("status")) == "selling"]
    if not selling:
        print("🔄 Recovery: nessuna carta SELLING.",flush=True)
        return
    print(f"🔄 Recovery: {len(selling)} carte SELLING",flush=True)
    for card in selling:
        print(f"   └─ {asset_id(card)} | offer={card.get('sale_offer_id')}",flush=True)
...
def worker():
    ...
    print("🔁 MINIMO TECNICO: da 0.0002 ETH, +0.0001 ETH",flush=True)
    ...
    print("🛡️ DUPLICATE ERROR: TRATTATO COME SELLING",flush=True)
        ...
        try:
            ...
            for card in cards:
                try:
                    ...
                    asset = asset_id(card)
                    print(f"❌ AutoSell {asset}: {e}",flush=True)
                    if asset:
                        update_card(asset,status="da_vendere",error=str(e))
            time.sleep(INTERVAL)
        except Exception as e:
...
def home():
    ...
    return jsonify({
        ...
        "storage":STATE_FILE,
        "cards":len(cards),
        "da_vendere":sum(1 for c in cards if norm(c.get("status")) in {"da_vendere","ready"}),
        "selling":sum(1 for c in cards if norm(c.get("status")) == "selling"),
